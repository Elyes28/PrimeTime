    ...
    if (teacher !== null && course !== null) {
      ...
      if (teacher_exist == 0) {
        ...
        await User.findByIdAndUpdate(
          { _id: teacher._id },
          { $addToSet: { courses_teaching: course._id } }
        );
  ...
  try {
    ...
    if (teacher !== null && course !== null) {
      ...
      if (teacher_exist == 0) {
        await Course.findByIdAndUpdate(
          { _id: req.params.id },
          { $addToSet: { teachers: req.body.teacher_id } }
        );
        await User.findByIdAndUpdate(
          { _id: req.body.teacher_id },
          { $addToSet: { courses_teaching: req.params.id } }
        );
        const c1 = await course.save();
        const course2 = await Course.findById(req.params.id).populate(
          "teachers"
        );
        console.log(course2);
        await res.json(course2);
    ...
    if (student !== null && course !== null) {
      ...
      if (student_exist == 0) {
        await Course.findByIdAndUpdate(
          ...
          { $addToSet: { students: req.body.student_id } }
        );
        await User.findByIdAndUpdate(
          { _id: req.body.student_id },
          { $addToSet: { courses_learning: req.body.course_id } }
        );
        const c1 = await course.save();

...
function ProductScreen(props) {
  ...
  /*  const addToCartHandler = () => {
    ...
    const quantity = existItem ? existItem.quantity + 1 : 1;
        Axios.get(`http://localhost:5000/products/${product._id}`).then((response) => {
  ...
  const getProduct = () => {
    Axios.get(`http://localhost:5000/products/slug/${slug}`).then(
      (response) => {
            ...
            <Grid container margin={5} spacing={6}>
              ...
              <Grid item md={2} xs={10}>
                <ListGroup>
                  ...
                  <ListItem>
                    <Typography> Color : {product.violonBody}</Typography>
                  </ListItem>
                </ListGroup>
              </Grid>
